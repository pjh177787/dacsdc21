...
typedef struct {
    ...
    XUltra_net_uio_map maps[ MAX_UIO_MAPS ];
} XUltra_net_uio_info;

/***************** Variable Definitions **************************************/
static XUltra_net_uio_info uio_info;
...
static int uio_info_read_name(XUltra_net_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XUltra_net_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XUltra_net_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XUltra_net_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XUltra_net_Initialize(XUltra_net *InstancePtr, const char* InstanceName) {
	XUltra_net_uio_info *InfoPtr = &uio_info;
