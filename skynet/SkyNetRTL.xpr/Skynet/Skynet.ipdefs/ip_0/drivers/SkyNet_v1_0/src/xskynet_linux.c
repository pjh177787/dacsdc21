...
typedef struct {
    ...
    XSkynet_uio_map maps[ MAX_UIO_MAPS ];
} XSkynet_uio_info;

/***************** Variable Definitions **************************************/
static XSkynet_uio_info uio_info;
...
static int uio_info_read_name(XSkynet_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XSkynet_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XSkynet_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XSkynet_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XSkynet_Initialize(XSkynet *InstancePtr, const char* InstanceName) {
	XSkynet_uio_info *InfoPtr = &uio_info;
